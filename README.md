...
- [Getting Started](#getting-started)
  - [Backend Setup (Python Agents)](#backend-setup-python-agents)
...
4. Run the orchestrator to start the agent system:
   ```shell
   python orchestrator.py
   ```
   This will prompt you to select a spec file from the `specs` directory and will run the manager, worker, and CEO agents based on the selected spec.
  
5. (Optional) Start the dashboard UI to monitor progress:
   ```shell
   python dashboard.py
   ```
   Then open your browser and navigate to `http://localhost:5000` to view a real-time dashboard of teams, workers, and API logs.

### Frontend Example App

The repository includes a minimal calendar app built with React, TypeScript, and Vite:

1. Navigate to the example app directory:
   ```shell
   cd minimal-calendar-app
   ```
